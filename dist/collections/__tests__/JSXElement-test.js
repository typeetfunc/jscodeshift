...
describe('JSXCollection API', function () {
  ...
  beforeEach(function () {
    ...
    nodes = [recast.parse(['var FooBar = require("XYZ");', '<FooBar foo="bar" bar="foo">', '  <Child id="1" foo="bar">', '     <Child />', '     <Baz.Bar />', '  </Child>', '  <Child id="2" foo="baz"/>', '</FooBar>'].join('\n'), { esprima: babel }).program];
  });
  ...
  describe('Traversal', function () {
    ...
    it('returns the child nodes of an JSXElement', function () {
      var children = Collection.fromNodes(nodes).findJSXElements('FooBar').childNodes();
    ...
    it('returns the child JSXElements of an JSXElement', function () {
      var children = Collection.fromNodes(nodes).findJSXElements('FooBar').childElements();
    ...
    it('returns a properly typed collection even if empty', function () {
      var children = Collection.fromNodes([]).findJSXElements('Foo').childElements();
  ...
  describe('Filtering', function () {
    it('filters elements by attributes', function () {
      var jsx = Collection.fromNodes(nodes).findJSXElements().filter(JSXElementCollection.filters.hasAttributes({ foo: "bar" }));
      expect(jsx.size()).toBe(2);
    });

    it('accepts callback functions as attribute filters', function () {
      var jsx = Collection.fromNodes(nodes).findJSXElements().filter(JSXElementCollection.filters.hasAttributes({ foo: function foo(v) {
          return ['bar', 'baz'].indexOf(v) > -1;
        } }));
      expect(jsx.size()).toBe(3);
    });

    it('filters elements by children', function () {
      var jsx = Collection.fromNodes(nodes).findJSXElements().filter(JSXElementCollection.filters.hasChildren('Child'));
      expect(jsx.size()).toBe(2);
    });
  });
  ...
  describe('Mappings', function () {
    it('gets the root names of JSXElements', function () {
      var names = Collection.fromNodes(nodes).findJSXElements().paths().map(JSXElementCollection.mappings.getRootName);
  ...
  describe('Mutation', function () {
    it('handles insertions before children correctly', function () {
      var childElement = b.jsxElement(b.jsxOpeningElement(b.jsxIdentifier('Bar'), [], true));
      var newChildElement = b.jsxElement(b.jsxOpeningElement(b.jsxIdentifier('Baz'), [], true));
      var literal = b.literal('\n  ');
      var ast = b.jsxElement(b.jsxOpeningElement(b.jsxIdentifier('Foo')), b.jsxClosingElement(b.jsxIdentifier('Foo')), [literal, childElement, literal, childElement, b.literal('\n')]);

      var children = Collection.fromNodes([ast]).childElements().at(1).insertBefore(newChildElement);
